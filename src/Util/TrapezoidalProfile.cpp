 ...
 *
 *	Ported from Team 341 / 254 Jared Russell TrajectorySmoother
 */
...
void TrapezoidalProfile::SetVelocity(double CurrentVel){
	m_CurrentVel = CurrentVel;
}

void TrapezoidalProfile::SetAcceleration(double CurrentAccel){
	m_CurrentAccel = CurrentAccel;
}

void TrapezoidalProfile::SetPosition(double CurrentPos){
...
//void TrapezoidalProfile::GetAccelTime()
...
void TrapezoidalProfile::GenerateAccelProfile(double ActualDistance, double ActualVel, double GoalVel){
	...
	if (ActualDistance > 0.0)
		...
		if (ActualDistance == 0.0)	//checks if distance is finished!
			return;
		else {
			GenerateAccelProfile(-ActualDistance, -ActualVel, -GoalVel);	//protection against incorrect signed velocities
...
//	std::cout <<"MaxAchievableVelocity: " << MaxAchievableVelocity<<std::endl;
    double FinalAccel;
	double MaxGoalVelocity; //Largest Velocity Outputed to Hit Goal
	if(MaxAchievableVelocity >= GoalVel){
		FinalAccel = -m_MaxDecel;
